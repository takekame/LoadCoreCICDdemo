...
if __name__ == '__main__':
    ...
    mw = Middleware(ip='54.249.194.228')
...
#    print("---- P10 getTestID ----")
    ...
    time.sleep(4)
    print("---- Checking the number of Registered UEs ----")
    res=mw.getStatFullCoreREG(testID)

    while True:
        res=mw.getStatFullCoreREG(testID)
        values = res['snapshots']
        reg_succeeded = values[-1]['values'][0][2]
        print("reg_succeeded: ", reg_succeeded)
        if reg_succeeded == '100':
            print("Confirmed 100 UEs Registration")
            break
        time.sleep(2)
